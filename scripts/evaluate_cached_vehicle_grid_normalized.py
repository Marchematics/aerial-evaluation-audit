...
CONF=[.10,.15,.20,.25,.30,.35,.40,.50]; IOU=[.25,.30,.40,.50]; THR=[.005,.0075,.010,.015,.020,.030]
CANDIDATES={
...
def main():
 ...
 ap=argparse.ArgumentParser(); ap.add_argument('--source',choices=list(configs),default=None); ap.add_argument('--candidate',choices=list(CANDIDATES),default=None); ap.add_argument('--out',type=Path,default=ROOT/'outputs/rule_grid_normalized'); ap.add_argument('--reference-only',action='store_true'); args=ap.parse_args()
 ...
 for source,(pred_path,classes) in selected.items():
     ...
     for thr in THR:
      for policy in ['include_all','exclude_without_ignore_protection','exclude_with_ignore_protection']:
       if policy=='include_all':
        ...
        gg=[x for x in g if x[4]/max(gt[name]['W'],gt[name]['H'])>=thr]; ignored=gt[name].get('ignore',[])
  ...
  for (conf,iou,thr,policy),(TP,FP,FN,count_sum,nimg) in agg.items():
   ...
   allrows.append({'source':source,'candidate':candidate_label,'confidence':conf,'iou':iou,'scale_threshold_norm':thr,'small_object_policy':policy,'matching':'greedy_iou','tp':TP,'fp':FP,'fn':FN,'precision':prec,'recall':rec,'f1':f1,'count_mae':count_sum/nimg if nimg else np.nan,'images':nimg})
 ...
 pd.DataFrame(allrows).groupby(['source','candidate','confidence','iou','scale_threshold_norm','small_object_policy'],as_index=False).first().to_csv(out/'count_metrics.csv',index=False)
