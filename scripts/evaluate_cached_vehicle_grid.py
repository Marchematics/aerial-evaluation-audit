...
def main():
 ...
 ap=argparse.ArgumentParser(); ap.add_argument('--source',choices=list(configs),default=None); ap.add_argument('--candidate',choices=list(CANDIDATES),default=None); ap.add_argument('--out',type=Path,default=ROOT/'outputs/rule_grid'); ap.add_argument('--reference-only',action='store_true'); args=ap.parse_args()
